...
class VectorDatabase():
    ...
    with self.get_connection() as conn:
      try:
        with conn.cursor() as cur:
          ...
          cur.execute(f"CREATE TABLE IF NOT EXISTS {table_name} (id BIGINT PRIMARY KEY DEFAULT nextval('{table_name}_id_seq'), sql text, description text, parameters text, explore_view varchar(50), model_name varchar(50), desc_vector vector(768))")
  ...
  def insert_record(self, sql, parameters, description, explore_view, model_name, desc_vector):
    with self.get_connection() as conn:
      try:
        with conn.cursor() as cur:
          insert_record = (sql, str(description), str(parameters), explore_view, model_name, str(desc_vector).replace(' ',''))
          cur.execute(f"INSERT INTO rag_test (sql, description, parameters, explore_view, model_name, desc_vector) VALUES ( %s, %s, %s, %s, %s, %s)", insert_record) 
      except Exception as e:
        print(e)
        conn.rollback()
        raise e
    ...
    with self.get_connection() as conn:
      try:
        with conn.cursor() as cur:
          ...
          cur.execute(f"SELECT sql, description, parameters, explore_view, model_name FROM rag_test ORDER BY desc_vector <-> %s LIMIT 1", select_record)
          return cur.fetchone()
      except Exception as e:
        print(e)
        conn.rollback()
